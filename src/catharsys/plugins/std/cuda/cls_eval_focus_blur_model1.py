...
class CEvalFocusBlurModel1:
    ...
    def __init__(
        ...
        *,
        ...
        # Load the focus blur evaluation CUDA kernel from ressources
        try:
            xKernelFlow = res.files(catharsys.plugins.std).joinpath("res").joinpath("EvalFocusBlurModel1.cu")
            with res.as_file(xKernelFlow) as pathKernelFlow:
                sKernelFlowCode = pathKernelFlow.read_text()
            # endwith
        except Exception as xEx:
            raise CAnyError_Message(sMsg="Error loading CUDA kernel for focus blur evaluation", xChildEx=xEx)
        # endtry

        iImgRowStride = self._tiSizeXY[0] * self._iImgChanCnt

        self._iDepthChanCnt: int = 1
        iDepthRowStride = self._iDepthChanCnt * self._tiRangeXY[0]

        sFuncBlurExp = (
            f"EvalFocusBlurModel1<{self._tiStartXY[0]}, {self._tiStartXY[1]}, "
            ...
            f"{iImgRowStride}, {iDepthRowStride}, "
            f"{self._iImgChanCnt}, {self._iDepthChanCnt}>"
        )
        ...
        try:
            ...
            raise CAnyError_Message(sMsg="Error compiling focus blur evaluation kernel", xChildEx=xEx)
        # endtry
    ...
    def Eval(
        self,
        *,
        _imgImage: np.ndarray,
        _imgDepth: np.ndarray,
        _fFocusDepth_mm: float,
        _fFocalLength_mm: float,
        _fApertureDia_mm: float,
        ...
        tArgs = (
            caImage,
            caDepth,
            cp.float32(_fFocusDepth_mm),
            cp.float32(_fFocalLength_mm),
            cp.float32(_fApertureDia_mm),
