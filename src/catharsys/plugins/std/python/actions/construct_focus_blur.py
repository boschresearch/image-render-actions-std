...
def GetDefinition():
    return {
        ...
        "sActionDTI": "/catharsys/action/std/blender/post-render/proc-focus-blur:1.0",
...
def Run(_xCfg: CConfigList) -> None:
    from .lib.cls_construct_focus_blur import CConstructFocusBlur
    from anybase import assertion

    assertion.FuncArgTypes()

    xConstruct = CConstructFocusBlur()

    def Process(_xConstruct: CConstructFocusBlur):
