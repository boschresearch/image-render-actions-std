...
def GetDefinition():
    return {
        ...
        "sActionDTI": "/catharsys/action/std/blender/post-render/proc-flow:1.0",
...
def Run(_xCfg: CConfigList) -> None:
    from .lib.cls_construct_flow import CConstructFlow
    from anybase import assertion

    assertion.FuncArgTypes()

    xConstruct = CConstructFlow()

    def Process(_xConstruct: CConstructFlow):
