...
def ResultData(xJobCfg: CConfigManifestJob):
    from .lib.cls_rs_result_data import CRsResultData

    return CRsResultData(xJobCfg=xJobCfg)
...
def Run(_xCfg: CConfigList) -> None:
    from .lib.cls_construct_rs import CConstructRS
    from anybase import assertion

    assertion.FuncArgTypes()

    xConstruct = CConstructRS()

    def Process(_xConstruct: CConstructRS):
