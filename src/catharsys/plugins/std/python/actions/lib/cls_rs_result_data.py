...
########################################################################################
class CRsResultData(CImageResultData):
    ...
    def ProcessImages(
        ...
        *,
        ...
        self._dicImages = {}
        lTrgAction = ["/catharsys/action/std/blender/post-render/proc-rs:1"]

        dicRdrTypeName = {"/catharsys/action/std/blender/post-render/proc-rs:1": "rs"}
        ...
        for dicCfg in lConfigs:
            ...
            sConstructInputType = "blender/construct/rs/input-id:1"
            dicData = dicCfg["mConfig"]["mData"]
            lConInType = config.GetDataBlocksOfType(dicData, sConstructInputType)
            if len(lConInType) == 0:
                raise Exception(
                    "No rolling shutter construction input type configuration "
                    "of type compatible to '{0}' given".format(sConstructInputType)
                )
            # endif
            sConInType = lConInType[0]

            #########################################################
            # FOURTH image dictionary level references:
            #       render output type, as defined in compositor
            #########################################################
            dicImgCfgRdrOutType = dicImgCfgRdrOut[sConInType] = {"mFrames": {}}
            #########################################################
