...
__global__ void EvalFlow(const float *aPos1, const float *aPos2, const int *aObjIdx1, const int *aObjIdx2, int *piIdxMapXY, float *pfSubPixIdx)
{
    ...
    if (bValid)
    {
        ...
        if (!is_zero(vVecTrg))
        {
            ...
            // pfSubPixIdx[iIdxPos + 2] = float(iMapX);
            // pfSubPixIdx[iIdxPos + 3] = float(iMapXR);
            // pfSubPixIdx[iIdxPos + 4] = float(iObjIdx1);
            // pfSubPixIdx[iIdxPos + 5] = float(aObjIdx2[iPosObjIdxR]);

            // fH = dot(vVecTrg, vVecR);
            // pfSubPixIdx[iIdxPos + 6] = float(fH);
            // pfSubPixIdx[iIdxPos + 7] = float(fH / sumsq(vVecR));

            // assign_float3(pfSubPixIdx, iIdxPos + 6, vVecR);
        }
